...
public class RecordingAudioContent {
	...
	private class Segment {
		...
		int[] chromaValues = null;
		int[] chromaCounts = null;
		...
		public float similarityTo(Segment b) {
			...
			float similarity = 0;
			if (a.length != b.length) { // division for normalization inside math.min
			...
			} else { // only one division at the end
				while (i < a.chromaValues.length && j < b.chromaValues.length) {
					if (a.chromaValues[i] < b.chromaValues[j])
						i++;
					else if (a.chromaValues[i] > b.chromaValues[j])
						...
						similarity += Math.min(a.chromaCounts[i++], b.chromaCounts[j++]);
				}
				similarity /= (float) a.length;
			}
			return similarity;
		}
